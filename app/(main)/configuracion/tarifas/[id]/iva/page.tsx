...
export default function TiposIVA() {
  ...
  const [tiposIVATarifa, setTiposIVATarifa] = useState<VATType[]>([])
  const [isLoading, setIsLoading] = useState(true)

  const [isNewIVAOpen, setIsNewIVAOpen] = useState(false)
  const [isEditIVAOpen, setIsEditIVAOpen] = useState(false)
  const [currentIVA, setCurrentIVA] = useState({ id: "", name: "", rate: 0 })
  const [nuevoIVA, setNuevoIVA] = useState({ name: "", rate: 0 })
  ...
  const handleCrearIVA = async () => {
    try {
      setIsSaving(true)
      // Usar nombres de campo de VATType
      // Eliminar tarifaId si addTipoIVA crea VATType global
      await addTipoIVA({
        name: nuevoIVA.name,
        rate: nuevoIVA.rate,
        isDefault: false
        // tarifaId: tarifaId // Eliminado
      })
      
      const tiposActualizados = await getTiposIVAByTarifaId(tarifaId)
      setTiposIVATarifa(tiposActualizados || [])
      
      // Restablecer usando nombres de VATType
      setNuevoIVA({ name: "", rate: 0 })
      setIsNewIVAOpen(false)
      
      toast({ title: "Tipo de IVA creado", description: "El nuevo tipo de IVA se ha creado correctamente" })
    } catch (error) {
  ...
  const handleActualizarIVA = async () => {
    try {
      ...
      await updateTipoIVA(String(currentIVA.id), {
        name: currentIVA.name,
        rate: currentIVA.rate
      })
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="p-4 space-y-4">
            <div className="space-y-2">
              <label htmlFor="nuevo-nombre" className="block text-sm font-medium text-gray-700">
                Nombre
              </label>
              <Input
                id="nuevo-nombre"
                value={nuevoIVA.name}
                onChange={(e) => setNuevoIVA({ ...nuevoIVA, name: e.target.value })}
                placeholder="Ej: IVA General, IVA Reducido..."
              />
            </div>

            <div className="space-y-2">
              <label htmlFor="nuevo-tasa" className="block text-sm font-medium text-gray-700">
                Tasa (%)
              </label>
              <Input
                id="nuevo-tasa"
                ...
                step="0.01"
                value={nuevoIVA.rate}
                onChange={(e) => setNuevoIVA({ ...nuevoIVA, rate: parseFloat(e.target.value) })}
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button 
                ...
                disabled={isSaving || !nuevoIVA.name}
              >
        ...
        <DialogContent className="sm:max-w-md">
          ...
          <div className="p-4 space-y-4">
            <div className="space-y-2">
              ...
              <Input
                id="edit-nombre"
                value={currentIVA.name}
                onChange={(e) => setCurrentIVA({ ...currentIVA, name: e.target.value })}
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                min="0"
                step="0.01"
                value={currentIVA.rate}
                onChange={(e) => setCurrentIVA({ ...currentIVA, rate: parseFloat(e.target.value) })}
              />
            </div>
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button 
                ...
                disabled={isSaving || !currentIVA.name}
              >
                {isSaving ? "Guardando..." : "Guardar"}
              </Button>
            </div>
