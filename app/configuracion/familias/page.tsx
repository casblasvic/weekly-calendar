...
export default function GestionFamilias() {
  ...
  useEffect(() => {
    ...
    loadFamilies();
  // Añadir todasLasTarifas a las dependencias (además de las otras)
  }, [getFamiliasByTarifaId, tarifaId, familiasTarifa, todasLasTarifas]);
        ...
        <table className="min-w-full divide-y divide-gray-200">
           ...
           <tbody className="bg-white divide-y divide-gray-200">
             ...
             {filteredAndSortedFamilies.map((family) => {
                console.log("[familias/page.tsx] Renderizando fila para familia:", JSON.stringify(family));
                return (
                  <tr key={family.id} className="table-row-hover">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900 whitespace-nowrap">{family.code}</td>
                    <td className="px-6 py-4 text-sm text-gray-700 whitespace-nowrap">{family.name}{!family.isActive && (<span className="inline-flex px-2 ml-2 text-xs font-semibold leading-5 text-yellow-800 bg-yellow-100 rounded-full">Deshabilitada</span>)}</td>
                    <td className="px-6 py-4 text-sm text-gray-500 whitespace-nowrap">{family.parentId ? (families.find(f => f.id === family.parentId)?.name || "(Desconocido)") : (<span className="text-gray-400">(Ninguna)</span>)}</td>
