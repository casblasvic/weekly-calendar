...
const OptimizedHoverableCell: React.FC<OptimizedHoverableCellProps> = memo(({
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (target.closest('[data-appointment-item]')) {
      clearHover()
      return
    }
    ...
    // Solo mostrar si no hay citas en esta posición exacta
  ...
  const handleDropWithExactTime = useCallback((e: React.DragEvent) => {
    if (active && isAvailable && !overrideForCell) {
      const exactTime = localPreview?.exactTime || time
      onDrop(e, day, exactTime, cabinId)
    }
  }, [active, isAvailable, overrideForCell, localPreview, time, onDrop, day, cabinId])

  // Renderizar preview del drag localmente
  const renderLocalDragPreview = () => {
    // Solo mostrar si:
    // 1. Hay un drag activo global
    // 2. Tenemos preview local (estamos sobre esta celda)
    // 3. Esta celda es válida para soltar
    if (!dragState.isActive || !localPreview || !isAvailable || overrideForCell) {
      return null
    }
    ...
    // Calcular la posición y tamaño del preview
    const [previewHour, previewMinute] = localPreview.exactTime.split(':').map(Number)
    const previewStartMinutes = previewHour * 60 + previewMinute
    const previewEndMinutes = previewStartMinutes + draggedItem.duration
    ...
    if (previewStartMinutes < slotEndMinutes && previewEndMinutes > slotStartMinutes) {
      ...
      // La altura debe ser siempre basada en la duración total de la cita
      // No importa si cruza entre slots
      const minuteHeight = cellHeight / slotDuration
      const height = draggedItem.duration * minuteHeight - 2
      
      return (
        <div
          className="absolute left-0 right-0 rounded shadow-lg border-2 border-white/50 pointer-events-none z-10"
          style={{
  ...
  return (
    ...
    >
      ...
      {/* Línea de hora al hacer hover (Granularidad) */}
      {showHover && isInteractive && !overrideForCell && (
        <TimeHoverIndicator 
      ...
      {/* Preview de drag renderizado localmente */}
      {renderLocalDragPreview()}
    </div>
  )
}, (prevProps, nextProps) => {
