...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  console.log('🔄 PUT /api/categories/[id] - INICIO');
  console.log('📋 Params recibidos:', resolvedParams);
  
  // Validar ID
  const paramsValidation = ParamsSchema.safeParse(resolvedParams);
  if (!paramsValidation.success) {
    console.log('❌ Error de validación de params:', paramsValidation.error);
    return NextResponse.json({ error: 'ID de categoría inválido.', details: paramsValidation.error.errors }, { status: 400 });
  }
  const { id: categoryId } = paramsValidation.data;
  console.log('✅ CategoryId validado:', categoryId);
  ...
  try {
    ...
    if (!validation.success) {
      ...
      console.log('📋 Errores detallados:', validation.error.format());
    ...
    console.log('✅ Datos validados exitosamente:', updateData);
