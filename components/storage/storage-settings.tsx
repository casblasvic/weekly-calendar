...
import StorageQuotaSettings from './storage-quota-settings';
...
export const StorageSettings = () => {
  ...
  const { getStorageStats } = useStorage();

  // Obtener estadísticas de uso total solo una vez al iniciar
  // usar useMemo en lugar de variables directas
  ...
  useEffect(() => {
    try {
      const storedConfig = getFromStorage('storageConfig', { 
        provider: 'local', 
        local: localConfig, 
    ...
    <div className="space-y-8">
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="quotas" className="mt-6">
          <StorageQuotaSettings showTotalInfo={true} />
        </TabsContent>
      </Tabs>
    </div>
  );
